...
echo '<hr>';
 //获取支付宝的反馈参数
    $order  = $_GET['out_trade_no'];    //获取支付宝传递过来的订单号
...
//请获取返回信息的这个字段discount的值，取绝对值，就是买家付款优惠的金额。即 原订单的总金额=买家付款返回的金额total_fee +|discount|.
	if($_GET['trade_status'] == 'WAIT_BUYER_PAY') {         //等待买家付款
		//放入交易状态是订单交易创建还未付款的数据库更新程序代码，也可不放入任何代码。
	file_put_contents('./WAIT.txt',print_r($_GET,true));	
	echo "success";
		
	}
	else if($_GET['trade_status'] == 'TRADE_FINISHED' ||$_GET['trade_status'] == 'TRADE_SUCCESS') {    //交易成功结束
        //放入订单交易完成后的数据库更新程序代码，请务必保证echo出来的信息只有success
		file_put_contents('./success.txt',print_r($_GET,true));
		...
		echo "success";
			...
			file_put_contents('./payerror.txt',print_r($_GET,true));		
		echo "fail";
	}
?>