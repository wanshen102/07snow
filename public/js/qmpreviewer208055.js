...
if(!g[h])
{
g.loadJsFileToTop(["$js_path$qmplayer/player1e7c35.js"]);
}
g.waitFor(function(){
return !!g[h];
},function(i){
...
e&&(e.style.left=e.style.top="-99999px");
...
},ossLog:ossLog,twoDCodeImgUrl:twoDCodeImgUrl};
})();
(function(a,c){
var d=window.QMPreviewerTmpl||(window.QMPreviewerTmpl={});
d.config={oUseHotKeys:{27:1,8:1,37:1,39:1}};
d.frame=a.TE(['<div id="preview_div_container" style="">','<div id="$sId$" orgcls="$sClass$" class="$sClass$" style="position:absolute;width:100%;height:100%;top:0;left:0;overflow:hidden;">','<a id="$sId$_focus" href="javascript:;"></a>','<iframe id="$sId$_downframe" style="display:none;"></iframe>','<div id="$sId$_scorllarea" class="scroll_area" style="position:absolute;width:100%;height:100%;"></div>','</div>','</div>']);
d.loading=a.T(['<div id="$sId$_loading" class="tip_loading"></div>','<div id="$sId$_msg" class="tip_msg"></div>']);
d.operator=a.T(['<div id="$sId$_minibar" class="minibar_area">','<div class="minibar unselect">','<a id="$sId$_minimize" orgcls="ico minimize" opt="minimize" title="\u6700\u5C0F\u5316"></a>','<span class="breakline"></span>','<a id="$sId$_close" orgcls="ico close" opt="close"></a>','</div>','</div>','<div id="$sId$_navbar" class="navigatebar unselect" opt="back">','<div id="$sId$_nav" orgcls="pv_navtxt"></div><i></i>','</div>','<div id="$sId$_mainbar" class="mainbar unselect" hide="auto">','<a id="$sId$_prev"  orgcls="pv_btn" opt="prev">','<span class="ico prev"></span>','</a> ','<a id="$sId$_next" orgcls="pv_btn" opt="next">','<span class="ico next"></span>','</a> ','<span id="$sId$_breakline" orgcls="breakline"></span>','<a id="$sId$_info" orgcls="pv_btn" opt="info">','<span class="ico info"></span>','</a> ','<a id="$sId$_list" orgcls="pv_btn" opt="list">','<span class="ico list"></span>','</a> ','<a id="$sId$_send" orgcls="pv_btn" opt="send">','<span class="ico send"></span>','</a> ','<a id="$sId$_down" orgcls="pv_btn" opt="down">','<span class="ico down"></span>','</a> ','<a id="$sId$_scan" orgcls="pv_btn" opt="scan">','<span class="ico qrcode"></span>','</a> ','</div>']);
...
d.img=a.TE(['<div id="$sId$_load" class="img_load">loading</div>','<div id="$sId$_fail" class="img_fail">','<p class="fail_info">$@$html($oFileInfo.sName$)$@$</p>','<div class="fail_disp"></div>','<div id="$sId$_err_notexist" style="color:#c00; margin:10px 0; display:none;">\u60A8\u9884\u89C8\u7684\u6587\u4EF6\u4E0D\u5B58\u5728\u3002<br>\u53EF\u80FD\u662F\u5DF2\u8FC7\u671F\uFF0C\u6216\u88AB\u53D1\u9001\u8005\u63D0\u524D\u5220\u9664\u3002</div>','</div>','$@$if($oFileInfo.sThumb$)$@$','<img id="$sId$_thumb" class="image" src="$oFileInfo.sThumb$"',b,'/>','$@$endif$@$','<img id="$sId$_img" class="image" src="','$@$if($oFileInfo.sUrl$)$@$','$oFileInfo.sUrl$','$@$else$@$','errurl','$@$endif$@$','" ',b,'/>','<div id="$sId$_bar" class="img_bar img_bar_hide">','<a class="img_bar_btn ico_pf_right" opt="rotate_r"></a>','<a id="$sId$_zoombtn" class="img_bar_btn ico_pf_zoomb" opt="zoom" zb_ttl="\u6700\u5927\u5C3A\u5BF8" zs_ttl="\u9002\u5E94\u7A97\u53E3" zs_cls="ico_pf_zooms"></a>','<div class="img_bar_txt">','$@$html($oFileInfo.sName$)$@$','</div>','</div>']);
...
d.menuselector_ctt=a.TE(['<li id="$_root_.sId$_item$_idx_$" opt="menu_select" sel="$_idx_$" class="menu_list_item">','$@$if($sThumb$)$@$','<div class="menu_list_img">','<table cellspacing="0" cellpadding="0"><tbody><tr><td>','<img src="$sThumb$" onerror="this.style.display=\x27none\x27;this.parentNode.className=\x27menu_list_img ico_fail_min\x27;"/>','</td></tr></tbody></table>','</div>','$@$else$@$','<div class="menu_list_img ico_fail_min"></div>','$@$endif$@$','<div class="menu_list_txt">$@$eval $_root_.oJs$.htmlEncode($sName$)$@$</div>','</li>']);
...
this._moOperator.enable({navbar:false,minibar:{minimize:this._calcViewerType(t)=="Music"},mainbar:{Video:1,Music:1}[this._msMode]||this._mbNoOpt?false:{app:this._mbShowApp,next:r?false:(u?true:"disable"),prev:r?false:(v?true:"disable"),breakline:this._mbShowBreakLine?!r:false,send:!!this._moEvents.fFrwd?(t.bIsTimeout==1?"disable":true):false,list:r?false:s,info:t.oInfo,scan:d&&t.bIsTimeout!=1&&!!t.sTwoDCodeUrl,down:this._mbShowDownBtn?(!t.sDown?false:(t.bIsTimeout==1?"disable":true)):false}},{Document:true,Compress:true}[this._calcViewerType(t)]);
...
var q=o.keyCode,p=true;
if(!QMPreviewerTmpl.config.oUseHotKeys[q])
{
return;
}
switch(q)
...
break;
default:
if({Callback:1,"Image":1,Document:1,Compress:1,Other:1}[this._msMode])
{
var r={37:"prev",39:"next"}[q];
r?this._operate(r):(p=false);
}
else if(this._moCurViewer&&!this._moCurViewer.key(q))
{
p=false;
}
}if(p)
...
var c=1500,d=0,g={minibar:{minimize:false,close:true},navbar:{nav:false},mainbar:{prev:true,next:true,breakline:true,list:false,send:true,scan:true,down:true,info:false}};
e.init=function(h,j){
var k=this;
f.init.call(k,h);
k._moBtnFulCfg={};
...
e.resize=function(){
var h=this;
h.adjustPos_().calcVisualRange_().resizeMenu_();
a.callBack(h._moEvents.fResizeVisualRange,[h.getVisualRange()]);
return h;
};
...
var e={mainbar:{app:true,prev:true,next:true,breakline:false,list:true,send:false,scan:true,down:true,info:false}};
c.init=function(f){
var g=this;
...
h._moListMenu=h._useComponent("Menu.Selector.Side",{sId:h._msId+"_list",sType:"list",oDomContainer:f.oDomFrame,oEvents:{fClick:a.funcProxy(h,h._menuOpt),fHide:a.funcProxy(h,h._unselectListBtn)}});
h._moScanMenu=h._useComponent("Menu.ScanSkin1",{sId:h._msId+"_scan",sType:"scan",oDomContainer:g,oEvents:{fClick:a.funcProxy(h,h._menuOpt),fHide:a.funcProxy(h,h._unselectScanBtn)}});
};
...
if(f===b||f=="mainbar")
{
var j=k._dom("optarea"),h,i,g;
if(i=k._moDomRange.offsetWidth)
{
j.style.left=0;
g=j.offsetWidth;
j.style.left=(i-g-20)+"px";
}
}
;return k;
};
c.calcVisualRange_=function(){
var i=this,h=i._moDomRange.clientWidth,g=i._moDomRange.clientHeight,f=(i._moDomMainBar.clientHeight||parseInt(a.getStyle(i._moDomMainBar,"height")))+1;
if(h&&g)
{
i._moVisualRange={nTop:f,nBottom:g,nLeft:0,nRight:h,nWidth:h,nHeight:g-f,nFullTop:0,nFullBottom:g,nFullLeft:0,nFullRight:h,nFullWidth:h,nFullHeight:g};
}
return i;
};
...
c.resize_=function(){
this._moDom.style.width=this._moDomCtt.scrollWidth+"px";
...
this._moDom.innerHTML=QMPreviewer.getView("menuside",e);
this._moDomCtt=this._dom("ctt");
this._initEvent(this._moDom,"click");
};
c.show=function(e,f){
...
if(g&&g.body.childNodes.length>0)
{
...
c.resize_=function(){
var g=this,h=g._moVisualRange;
if(h)
{
var i=g._moFileInfo.sName.split(".").pop(),f=(e||{xls:1,xlsx:1,ppt:1,pptx:1}[i])?(g._moDomDocTitle?g._moDomDocTitle.clientHeight:0):0;
g._resizeHeight();
g._moDom.style.top=h.nTop+"px";
g._moDom.style.width="100%";
g._moDom.style.left=h.nLeft+"px";
try{
}
catch(j)
{
...
if(f)
{
var e=(f.nWidth<840?f.nWidth:840);
this._moDom.style.top=f.nFullTop;
this._moDom.style.height=f.nFullHeight+"px";
this._moDom.style.width=e+"px";
this._moDom.style.left=f.nLeft+(f.nWidth-e)/2+"px";
}
};
...
this._moNavOpter.enable({navbar:{nav:this._moPath[this._moPath.length-1].sName},minibar:{minimize:f.sType=="music"},mainbar:{next:false,prev:false,breakline:false,list:false,send:false,scan:false,info:false}},{doc:true}[f.sType]);
this._moViewer=e;
...
c._lazyLoadPic=function(){
var g=this._moDom,e=0,f=0;
a.E(a.SN("preview_unloaded_img"),function(j,h,i){
a.contain(g,j)&&a.attr(j,"tmpsrc")&&setTimeout(function(){
var k=a.attr(j,"tmpsrc");
if(k)
{
j.src=k;
j.removeAttribute("tmpsrc");
}
j=k=b;
},(e=e+(f++==0?2000:(f>4?100:500))));
i-1==h&&(g=b);
});
};
c._readDir=function(){
...
this.installFileViewer_(this._useComponent(h,{sId:this._msId+"_"+h,oTip:this._moTip,oOperator:this._moNavOpter,oDomContainer:this._moDomContainer,oVisualRange:this._moVisualRange,oFileInfo:e}));
};
...
c.resize_=function(){
var e=this._moVisualRange;
if(e)
{
this._moDom.style.top=e.nTop+"px";
this._moDom.style.height=e.nHeight+"px";
this._moDom.style.width=e.nWidth+"px";
this._moDom.style.left=e.nLeft+"px";
}
};
c.installFileViewer_=function(e){
var f=e.getFileInfo();
this.uninstallFileViewer_();
this._moNavOpter.enable({navbar:{nav:this._moPath[this._moPath.length-1].sName},minibar:false,mainbar:{next:false,prev:false,breakline:false,list:false,send:false,scan:false,info:false}},{doc:true}[f.sType]);
this._moViewer=e;
this._moNavOpter.resize();
this._moNavOpter.setFileInfo(a.extend({sName:f.sName},f.oInfo));
...
i._initEvent(i._moDom,"mouseover|mouseout|mousedown|mousemove|dblclick");
...
var e=this;
this._mnTimer=setInterval(function(){
e._showImgOrFail(e._imgSize())&&e._clearDetect();
},300);
};
c._show=function(f,e){
var k=this._moVisualRange,h=this._calcBorder(),i=Math.floor(k.nLeft+(k.nWidth-f-h)/2),j=Math.floor(k.nTop+(k.nHeight-e-h)/2);
this._moDom.style.width=f+"px";
this._moDom.style.height=e+"px";
this._mbIsAllowTransition&&a.addClass(this._moDom,"layer_img_ts");
if(!this._mbIsZoomMode)
{
var g=!this._mbIsRotate;
_nBottom=k.nFullHeight-(g?j:Math.floor(k.nTop+(k.nHeight-f-h)/2))-(g?e:f)-(h/2);
this._moDomBar.style.width=(g?f:e)+"px";
this._moDomBar.style.left=(g?i:Math.floor(k.nLeft+(k.nWidth-e-h)/2))+(h/2)+"px";
this._moDomBar.style.bottom=(_nBottom>0?_nBottom:0)+"px";
}
else{
this._moDomBar.style.cssText="";
}
a.posShow(this._moDom,i,j);
};
c._showLoading=function(){
a.posHide(this._moDomCurShow);
a.posShow(this._moDomCurShow=this._moDomLoad);
...
var f=this._calcBorder(),o=this._moVisualRange,m=(o.nWidth-this._mnRangeDelta-f)||this._mnRangeDef,l=(o.nHeight-this._mnRangeDelta-f)||this._mnRangeDef,i=this._mbIsRotate?l:m,h=this._mbIsRotate?m:l,k=e.nWidth,j=e.nHeight,n=k/i,g=j/h;
if(n>1||g>1)
{
if(n>g)
{
...
a.show(this._dom("zoombtn"),true);
}
else{
a.show(this._dom("zoombtn"),false);
}
this._show(k,j);
};
...
{
var g=e.naturalWidth||e.width,f=e.naturalHeight||e.height;
...
this._operator().enable(this._moOptBtnCfg).resize();
...
this._mbIsAllowTransition&&this._moDomBar&&a.setClass(this._moDomBar,"img_bar"+(this._mbIsZoomMode?" zoom_mode":"")+(e===true?" no_ts":""));
};
c._hideBar=function(e){
this._clearHideBarTimer();
this._moDomBar&&a.setClass(this._moDomBar,"img_bar img_bar_hide"+(this._mbIsZoomMode?" zoom_mode":"")+(e===true?" no_ts":""));
};
...
if(this._mCursorX!=-1)
{
if(!this._mbIsZoomMode&&Math.max(Math.abs(this._mCursorX-f),Math.abs(this._mCursorY-g))>5&&(new Date()-(this._mnRotateTime||0)>200))
{
this._showBar();
...
!this._mbIsZoomMode&&!a.contain(this._moDom,f)&&!a.contain(this._moDomBar,f)&&this._delayHideBar();
};
...
if(f)
{
this._moDomMusicContainer.style.top=(f.nTop+Math.max(0,f.nHeight-this._moDomMusicContainer.clientHeight)/2)+"px";
...
c._show=function(){
var e=this._moVisualRange;
a.posShow(this._moDom,Math.floor(e.nLeft+(e.nWidth-(this._moDom.clientWidth||0))/2),Math.floor(e.nTop+(e.nHeight-(this._moDom.clientHeight||0))/2));
this.log_(1);
};
c._hide=function(){
a.posHide(this._moDom);
