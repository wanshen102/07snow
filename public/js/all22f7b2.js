...
var e=d.disabled;
d.disabled=a;
if(!gbIsIE&&d.tagName.toUpperCase()=="A")
{
if(e!=a)
{
...
{
var p=getTop(),k=d||document,h=typeof b=="function",m,n,l=p.loadJsFile,q=p.getRes(e),j=l._oDatas||(l._oDatas={});
if(QMDistributeDomain.isRelativeUrl(q))
{
q=QMDistributeDomain.addHost(q);
}
if(typeof (f)=='boolean')
{
f={bAutoRemove:f};
...
if(a)
{
for(var o=GelTags("script",k),s=o.length-1;s>=0;s--)
{
if(o[s].src.indexOf(q)!=-1)
{
if(h)
{
var r=o[s].getAttribute("_key_");
if(j[r]===true)
{
callBack.call(o[s],b);
}
...
{
if(c.crossOrigin!==false&&l.checkCrossOrigin(q))
{
c.crossOrigin='*';
}
else{
...
if(f.bReload)
{
q+=(q.indexOf('?')!=-1?'&':'?')+'r='+Math.random();
}
p.E(c,function(t,i){
n.setAttribute(i,t);
});
var r=p.unikey();
n.setAttribute("_key_",r);
j[r]=[];
function g()
{
var i=this,t=i.getAttribute("_key_");
callBack.call(i,b);
p.E(j[t],function(u){
u();
});
...
i.onreadystatechange=i.onload=i.onerror=null;
}
(GelTags("head",k)[0]||k.documentElement).appendChild(extend(n,{onerror:function(){
if(c.crossOrigin)
{
debug('crossOrigin error file:'+q);
p.ossLogCustom('delay','all','corsError',e);
p.LogKV({sValue:'getinvestigate|jsload|cors|jserr'});
c.crossOrigin=false;
f.bReload=true;
p.removeSelf(n);
...
debug('file load error:'+q);
}
},onload:g,onreadystatechange:function(){
var i=this;
({loaded:true,complete:true}[i.readyState])&&g.call(this);
}},{type:"text/javascript",charset:c.charset||"gb2312",src:c.crossOrigin?(q.indexOf('?')>0?q+'&r=o':q+'?r=o'):q}));
return n;
}
...
var q=n.id||unikey("_"),h=[n.attrs],m=[n.header],k=[],c=cacheByIframe._bIsSupportVirtual&&(n.virtual!==false);
...
{
var d=getTop();
d.loadJsFile("$js_path$qmtip207d36.js",true);
d.waitFor(function(){
return d.QMTip;
},function(e){
if(e)
...
{
var b=getEventTarget(a),d=b.tagName.toLowerCase(),c=QMDistributeDomain.removeHost(b.href||''),c=QMDistributeDomain.removeDefaultHost(c);
...
{
var b=getEventTarget(a),d=b.tagName.toLowerCase(),c=b.href;
...
var q='/cgi-bin/$cgi$?folderid=$folderid$&folderkey=$folderkey$$s$&t=$t$&mailid=$mailid$$cache$&sid=$sid$',k,o,n,p,m="readmail";
if(a)
{
o="readmail&s=draft";
}
...
{
o=f==100?"compose_card&s=draft":"compose&s=draft";
}
else if(h==14)
{
o="compose_group&groupdraft=true&s=draft";
}
else if(h=="9")
{
q=[location.protocol,location.protocol=="https:"?"//ws.mail.qq.com":"//msgopt.mail.qq.com",q].join("");
o="sms_list_v2";
m="readtemplate";
}
...
if(e==3)
{
o="readmail_ad_conversation";
}
else{
o="readmail_conversation";
}
break;
case '@':
o="readmail_group";
break;
case 'A':
return c?'':QMMLCache.folder('1',{dr:'ad'},true).split('#')[0];
break;
default:
o=e==3?"readmail_ad":"readmail";
break;
}k=true;
}
if(b)
{
n=["&newwin=true","&compose_new=compose"][e?0:1];
}
else{
n=["","&s=from_unread_list","&s=from_star_list"][d!=1&&d!=2?0:d];
}
...
p=T(q).replace({cgi:m,mailid:i,folderid:h,folderkey:g,t:o,s:n,sid:getSid(),cache:l?T("&mode=pre&maxage=$maxage$&base=$base$&ver=$ver$").replace({maxage:rdVer.maxage(i),base:rdVer("BaseVer",0),ver:l}):""});
return c?p.split("?")[1]:p;
};
...
{
if(d.indexOf("/cgi-bin/bizmail")==-1)
{
...
var f,a=["u","1","2","3","4"],e=getEventTarget(b),d=function(g){
if(g&&g.getAttribute)
{
...
if(!b)
{
insertHTML(Scale.getContainer(),"afterBegin",T(['<table id="$id$" cellspacing=0 cellpadding=0 border=0 ','style="position:absolute;top:$top$px;left:0;width:100%;display:none;z-index:9999;">','<tr><td align="center">','<table id="$id$_pos" cellspacing=0 cellpadding=0 border=0 class="autosave autosave_txt" style="height:20px;"><tr>','<td style="width:2px;"></td>','<td id="$id$_img" style="padding:0 0 0 5px;">','<img src="$image_path$ico_loading104474.gif" style="width:16px;height:16px;vertical-align:middle;">','</td>','<td id="$id$_plan" valign=center style="padding:0 0 0 5px;">','<div style="font:1px;border:1px solid white;width:104px;text-align:left;">','<div id="$id$_plan_bar" style="font:1px;background:#fff;height:8px;margin:1px 0;width:50%;"></div>','</div>','</td>','<td id="$id$_plan_info" style="padding:0 0 0 5px;"></td>','<td id="$id$_plan_rate" style="width:40px;text-align:right;padding:0;"></td>','<td id="$id$_info" style="padding:0 0 0 5px;"></td>','<td id="$id$_cancel" style="padding:0 0 0 5px;">','[<a onclick="getTop().cancelDoSend();" nocheck="true" style="color:white;">\u53D6\u6D88</a>]','</td>','<td style="padding:0 0 0 5px;"></td>','<td style="width:2px;"></td>','</tr></table>','</td></tr>','</table>']).replace({id:c,top:isNaN(a)?(getTop().bnewwin?0:45):a,image_path:getPath("image",true)}));
b=S(c,getTop());
}
else if(!isNaN(a))
{
...
setVerifyCallBack();
loadingBox({model:"\u9A8C\u8BC1\u7801",js:"$js_path$qmverify210d8b.js",oncheck:function(){
...
loadJsFileToTop(["$js_path$com/kits/qmeditor/qqmail/release/editor2275b0.js"]);
loadingBox({model:"\u53D1\u4FE1",js:["$js_path$libcompose20f747.js","$js_path$qmaddrinput22e1ae.js"],oncheck:function(){
...
if(S("tips",a))
{
S("tips",a).innerHTML=T(['<img src="$images_path$ico_loading3104474.gif" align=absmiddle>',' \u6B63\u5728\u6536\u53D6...&nbsp;\u7CFB\u7EDF\u5C06\u5728\u540E\u53F0\u81EA\u52A8\u6536\u53D6\uFF0C\u60A8\u53EF\u4EE5\u79BB\u5F00\u6B64\u9875\u9762\uFF0C\u7A0D\u540E\u56DE\u6765\u67E5\u770B\u6536\u53D6\u7ED3\u679C\u3002']).replace({images_path:getPath("image",true)});
}
recvPopHidden(b);
}
...
QMAjax.send("/cgi-bin/mail_mgr?mailaction=read_all&t=unreadmail_reg_data&loc=setFolderUnread,,,32",{method:"POST",content:T('sid=$sid$&folderid=$folderid$&groupid=$groupid$$flags$').replace({sid:_sSid,folderid:_asFolderId!="all"?_asFolderId:"1&folderid=3&folderid=8&folderid=9&folderid=11&folderid=personal&folderid=pop&folderid=subscribe",groupid:_asGroupId,flags:_asFlag?_asFlag:""}),onload:function(_abIsOk,_asParam){
...
if(attr(e,"t")=="5")
{
c.QMReadMail.addCalEvent(e,d,b);
}
else if(attr(e,"t")=="6")
{
getTop().openMenu(e);
...
if(d(m))
{
p="<span style='border-bottom:1px dashed #ccc;z-index:1' t='6' onclick='return false;' data='"+l+"'>"+l+"</span>";
...
if(d)
{
b.QMAjax.send(b.T("/cgi-bin/webapnscheck?sid=$sid$&key=$key$&action=pushphoneno&phoneno=$phoneno$").replace({sid:b.getSid(),key:"",phoneno:encodeURIComponent(c)}),{method:"POST",onload:function(){
...
if(b!="preview"&&r.indexOf('/cgi-bin/readmail?')==-1&&r.indexOf('t=mail_list_ad')==-1&&r.indexOf('t=unreadmail')==-1)
{
return true;
}
var g=a.parentNode;
...
{
loadJsFile("$js_path$qmtip207d36.js",true);
...
while(o.tagName.toUpperCase()!="TABLE")
{
o=o.parentNode;
}
MLIUIEvent(n,a,c);
...
if(b)
{
QMAjax.send(T("/cgi-bin/exbook_mgr?sid=$sid$&fname=&optype=mailfilter").replace({sid:getSid()}),{method:"GET",headers:{"If-Modified-Since":"0","Cache-Control":"no-cache, max-age=0"},onload:function(c,d){
...
{
cacheByIframe(a&&a.defcss==false?[]:[["css","",getRes("$css_path$comm2275b0.css")],["css",getPath("style"),"skin"]],extend({className:"menu_base_if",transparent:false,destroy:false},a,{win:b,header:["<script>",getTop.toString().replace(/[\r\n]/g,""),"<\/script>",a&&a.header||""].join(""),onload:function(c){
...
{
var b=new QMDialog({sId:"LoAdINgBOx",sTitle:a.model+"\u6A21\u5757\u52A0\u8F7D\u4E2D...",nWidth:400,nHeight:187,sBodyHtml:T(['<div style="text-align:center;padding:58px; padding-bottom:78px;">','<img id="load" src="$images_path$ico_loading2104474.gif">','<span id="err" style="display:none;">$model$\u6A21\u5757\u52A0\u8F7D\u5931\u8D25</span>','</div>']).replace(extend(a,{images_path:getPath("image")})),onclose:function(){
b=null;
}});
if(a.js)
{
...
loadJsFile("$js_path$qmlinkman1e7c35.js",true,e,b);
loadJsFile("$js_path$qmaddress2117f7.js",true,e,b);
}
...
if(!b.QMPlayer)
{
loadJsFileToTop(["$js_path$qmplayer/player1e7c35.js"]);
}
waitFor(function(){
return !!b.QMPlayer;
},function(c){
...
if(!c.QMPlayer)
{
loadJsFile("$js_path$qmplayer1e7c35.js",true,c.document);
}
return b(a);
}
function playUrl(a)
...
{
var b=getTop();
b.loadCssFile("$css_path$webpushtip2275b0.css",true);
b.loadJsFile("$js_path$qmwebpushtip1e7c9d.js",true,b.document,function(){
b.QMWebpushTip.open(a);
});
b.loadJsFile("$js_path$qmwebpush1e7c36.js",true,b.document);
}
...
c&&showInfo("\u52A0\u8F7D\u4E2D\uFF0C\u8BF7\u7A0D\u5019...");
getTop().loadJsFile("$js_path$gift/app1e7caa.js",true,getTop().document,function(){
hiddenMsg();
if(b.indexOf("msg:")==0)
{
...
return (''+a).replace(ossLogCustom._pasteLogReg,'_');
};
...
return this.GDT.isGDTADUser(a)||b;
...
var b=TE(['<img id="ad_pingurl" link="$pingurl$" style="position:absolute;z-index:-1;"/>','$@$if($monitor_url$)$@$','<span style="background:url($monitor_url$);"></span>','$@$endif$@$','$@$if($mail_monitor_url$)$@$','<span style="background:url($mail_monitor_url$);"></span>','$@$endif$@$','<a id="$id$" href="$link_to$" target="_blank"  onclick="getTop().AD.postADlog(\'img\',\'$pos$\')" ','style="height:83px;overflow:hidden;display:block;margin-bottom:0px;">','<img src="$resource_url$" onload="getTop().AD.handleImgSucc(\'$loc$\', window)" onerror="getTop().AD.handleImgError(\'$loc$\', window)"/>','</a>']),c=TE(['<a href="$link_to$" target="_blank" onclick="getTop().AD.postADlog(\'img\',\'$pos$\')">','<img id="ad_pingurl" link="$pingurl$" style="position:absolute;z-index:-1;"/>','<img id="$id$" src="$resource_url$" width="$width$" height="$height$" onload="getTop().AD.handleImgSucc(\'$loc$\', window)" onerror="getTop().AD.handleImgError(\'$loc$\', window)"/>','$@$if($monitor_url$)$@$','<span style="background:url($monitor_url$);"></span>','$@$endif$@$','$@$if($mail_monitor_url$)$@$','<span style="background:url($mail_monitor_url$);"></span>','$@$endif$@$','</a>','$@$if($btencentpromot$)$@$','<div class="txt_right graytext" style="margin-top:4px;">\u817E\u8BAF\u63A8\u5E7F</div>','$@$endif$@$','<style>','.ad_btn_close{position:absolute; top:5px; right:5px; line-height:0; text-decoration:none; background:#aaa; width:12px; height:12px;  border:1px solid #999;}','.ad_btn_close:hover{border-color:#888;background-color:#999;}','</style>','$@$if($bCloseBtn$)$@$',' <a class="ad_btn_close" onclick="closeAD(\'$pos$\')"><img src="$img_path$ico_closetip.gif"></a>','$@$endif$@$']);
if(a==1)
{
return b;
}
...
try{
var n=this,m=this._getADDomByloc(c,d),o=d+"_showed",h=location.protocol=="https:",p=h?T("https://stockweb.mail.qq.com/p?oid=$oid$&cid=$cid$&loc=$loc$&pdomain=p.l.qq.com"):T("http://p.l.qq.com/p?oid=$oid$&cid=$cid$&loc=$loc$"),g=attr(m,"bgimg")=="1"?true:false,f=attr(m,"closebtn")=="1"?true:false,i=attr(m,"tencentpromot")==1?true:false,j=attr(m,"pos");
if(m&&b.resource_url)
{
var l=m.parentNode;
...
show(l,1);
insertHTML(l,"beforeEnd",_sADHtml.replace(extend(b,{id:o,loc:d,img_path:getPath("image"),bIsIE:gbIsIE,btencentpromot:i,bCloseBtn:f,pingurl:p.replace(b),pos:j})));
debug(d);
...
},GDT:{_msTxtTmpl:TE(['<dl class="qm_ad_promotion_list">','<dt class="qm_ad_promotion_title"><a  class="qm_ad_promotion_title_link" $olink$>$title$</a></dt>','<dd class="qm_ad_promotion_detail">$desc$</dd>','<dd class="qm_ad_promotion_site"><a $olink$ class="qm_ad_promotion_site_link" >$showurl$</a></dd>','</dt>','</dl>']),_msBottomTmpl:'<p class="qm_ad_promotion_bottom">\u817E\u8BAF\u63A8\u5E7F</p>',_moADLoc:[],_msGDTTimeout:"",_moADLocData:[],_moOssLogData:[],init:function(b,a){
...
{
loadJsFileToTop("http://qzs.qq.com/qzone/biz/comm/js/qbs.js");
}
b["GDTBackupCallback"]=function(e){
var h={},f=0,g=3,i="";
function d(j,k)
{
...
h.title=j.txt;
h.desc=j.desc;
h.olink=" href=\""+j.rl+"\" target=\"_blank\"";
h.showurl=j.rl;
i+=c._msTxtTmpl.replace(h);
}
});
c._render(i);
};
this._postADCode();
},isGDTADUser:function(a){
try{
var b=S("qqmail_readmail_AD_container",getMainWin())!=null;
return b;
}
catch(c)
{
...
d._moSendReqTime=new Date()-d._moStartTime;
d._mnProcessId=21;
var j="",i={},h=false;
if(g&&g.ret==true)
{
...
i.olink=e.olink;
d._mnProcessId=28;
j+=d._msTxtTmpl.replace(i);
d._mnProcessId=29;
d._postOssLogData(e);
d._mnProcessId=40;
});
if(d._msGDTTimeout!="timeout")
{
d._mnProcessId=41;
d._render(j);
d._mnProcessId=55;
...
d._moCompleteExpireTime=new Date()-d._moStartTime;
d._postTestLog("timeout",{expirecompletetime:d._moCompleteExpireTime,sendreqtime:d._moSendReqTime,loadtime:d._moLoadTime});
h=true;
}
}
catch(k)
{
d._postTestLog("error",{errmsg:encodeURIComponent(k.message)});
h=true;
}
}
else{
d._handleDefault(false);
}
if(!h)
{
d._msGDTTimeout="ok";
d._moCompleteTime=new Date()-d._moStartTime;
d._postTestLog("ok",{completetime:d._moCompleteTime,sendreqtime:d._moSendReqTime,loadtime:d._moLoadTime});
}
};
d._msGDTTimeout="";
d._moADLoc=[];
d._moADLocData=[];
d._moOssLogData=[];
E(GelTags("gdtqqmailad",this._moWin.document),function(e){
var i={},l=attr(e,"ss"),h=attr(e,"posid"),g=attr(e,"count")||3,j=e&&e.parentNode,k=attr(j,"id");
i.posId=h;
i.container=null;
i.count=g;
i.context={ss:l};
...
i.onComplete=b;
d._moADLoc.push(k);
d._moADLocData.push(i);
});
if(d._moADLoc.length>0)
{
d._moStartTime=new Date();
d._mnProcessId=0;
setTimeout(function(){
if(d._msGDTTimeout!="ok")
{
d._msGDTTimeout="timeout";
d._handleDefault(true);
}
},3*1000);
try{
waitFor(function(){
...
if(e)
{
d._moLoadTime=new Date()-d._moStartTime;
d._mnProcessId=20;
GDT.load(d._moADLocData);
d._moWin.GDT=GDT;
}
else{
d._postTestLog("GDTErr",{errmsg:"no GDT loaded"});
}
});
}
catch(f)
{
d._postTestLog("error",{errmsg:encodeURIComponent(f.message)});
}
}
},_handleDefault:function(a){
var c=this,d="http://qzs.qq.com/qzone/biz/gdt/display/mailbackorder.js",b=loadJsFile(d+"?"+Math.random(),false,this._moWin.document,null,{charset:"utf-8"});
ossLog("realtime","all","stat=gdt_log_show&adshow=noad&timeout="+(a?"1":"0"));
},_render:function(a,b){
var e=this,c=null,f=a+e._msBottomTmpl;
e._mnProcessId=42;
if(!b)
{
E(e._moADLoc,function(g){
e._mnProcessId=43;
c=S(g,e._moWin);
e._mnProcessId=44;
rmClass(c,"hidden");
...
var d=this._getADDomByPosId(this._moWin,b);
c=d.parentNode;
rmClass(c,"hidden");
attr(c,"AD","1");
show(c,1);
...
e._mnProcessId=49;
AD.setAdFlag(e._moWin);
e._mnProcessId=50;
AD.adjustADDisp(e._moWin);
e._mnProcessId=51;
},_getMailInfo:function(a){
var b=a&&a.mailinfo,e="",d="",c="";
if(b.nLen&&b.nLen>0)
{
e=b.oSubMails[0][1].subject||"";
d=b.oSubMails[0][1].from.addr;
c=b.oSubMails[0][0].cmailid;
}
else{
e=b.subject||"";
d=b.from.addr;
...
return {mailid:c,subject:e,sender:d};
},_getADDomByPosId:function(a,b){
var c=null;
E(GelTags("gdtqqmailad",a.document),function(d){
_sADPosId=attr(d,"posid")||"";
if(_sADPosId==b)
{
c=d;
}
...
return c;
},_postOssLogData:function(a){
var b=T("&adtitle=$txt$&adid=$cl$");
this._moOssLogData.push(b.replace(a));
},_postOssLog:function(a){
var c=this._getMailInfo(this._moCfg).sender,d=this._getMailInfo(this._moCfg).subject,b=T("stat=gdt_log_show&logtype=$type$&subject=$subject$&sender=$sender$");
ossLog("realtime","all",b.replace({type:a,sender:c,subject:encodeURI(d)})+this._moOssLogData.join(""));
this._moOssLogData=[];
},_postTestLog:function(b,a){
var c=T("stat=gdt_log_err&processid=$processid$&erraction=$erraction$&errmsg=$errmsg$&expiretime=$expirecompletetime$&completetime=$completetime$&loadtime=$loadtime$&sendreqtime=$sendreqtime$");
ossLog("realtime","all",c.replace(extend(a,{processid:this._mnProcessId,erraction:b})));
}}};
...
b.loadCssFile("$css_path$login/addAccount2275b0.css",true,b.document);
...
return function(g,f){
var c=a.S("importFrame"),d=a.S("div_stepPassword"),e=a.S("div_username"),b=a.S("geCancelBtn");
if(c)
{
c.style.height=f+'px';
d.style.height=(f-66)+'px';
if(f>=171&&f<=290)
{
...
var c=this,b=c._oDialog;
_oTop.loadJsFile("$js_path$safeauth1e7c35.js",true);
b.setHtml("_body_",c._oTmpl.replace({"step":a.step,"username":a.username,"uInfo":"other"}));
new (c.BindPwdEvents)(b.S("ipt_pwd"),b.S("capTip"),c.enterPress);
b.S("geConfirmBtn").onclick=function(){
c.goStep({"step":"3","username":a.username});
};
b.S("geCancelBtn").onclick=function(){
c.goStep({"step":"1","username":a.username});
};
...
c.QMAjax.send(c.T(["/cgi-bin/foldermgr?sid=#sid#&enter=new&fun=createpop&folderid=&acctid=0&taskid=&step=","&fromtips=&popfolder=#popmail#&popnickname=#popnickname#&popmail=#popmail#&addstep=2&ptfun=addother&poppwd=","&popsvr=&popport=&timeflag=1&delflag=0&isenddef=1&needspam=1&smtpneedpwd=0&showsmtppanel=0","&agentsend=0&smtpsvr=&smtpport=25&nosmtppwd=#nosmtppwd#&smtppwd=&t=mailpotl.json"],"#").replace({sid:c.getSid(),popmail:a.sLoginaccount,popnickname:encodeURIComponent((c.S("useralias")&&c.S("useralias").innerHTML)||""),nosmtppwd:encodeURIComponent("\u6CBF\u7528\u90AE\u7BB1\u5BC6\u7801")}),{content:"secpwd="+a.secpwd,method:"POST",onload:function(d,f){
if(d)
{
var g=c.evalValue(f);
try{
...
{
b.showInfo("\u6B63\u5728\u9A8C\u8BC1\u5176\u4ED6\u90AE\u7BB1\u5E10\u53F7...",1000);
var c=_oSelf._oDialog.S("div_username").innerHTML||"",e=_oSelf._oDialog.S("ipt_pwd").value||"",f=_oSelf._oDialog.S("ipt_popserver").value||"",d=_oSelf._oDialog.S("ipt_port").value||110;
b.QMAjax.send(b.T("/cgi-bin/foldermgr?sid=#sid#&enter=new&fun=createpop&folderid=&acctid=0&step=&fromtips=&popmail=#popmail#&popfolder=#popmail#&popnickname=&popsvr=#popsvr#&popport=#popport#&timeflag=1&delflag=&isenddef=1&needspam=1&smtpneedpwd=0&showsmtppanel=0&agentsend=&smtpsvr=&smtpport=25&nosmtppwd=&smtppwd=&taskid=#taskid#&t=mailpotl.json","#").replace({sid:b.getSid(),popmail:encodeURIComponent(c),popsvr:encodeURIComponent(f),popport:encodeURIComponent(d),taskid:a||""}),{content:T('poppwd=$poppwd$').replace({poppwd:encodeURIComponent(_oSelf.encrypt(e))}),method:"POST",onload:function(g,h){
if(g)
{
var i=b.evalValue(h);
try{
if(i.waiting=="true")
{
b.showInfo("\u6B63\u5728\u9A8C\u8BC1\u5176\u4ED6\u90AE\u7BB1\u5E10\u53F7...");
setTimeout(function(){
_oSelf.doAction(i.taskid);
},5000);
}
else if(i.succ=="0")
{
_oSelf.showError(i.failtype);
}
else if(i.succ=="1")
{
_oSelf.succ({"step":"1","username":c,"folderid":i.folderid,"acctid":i.acctid});
}
}
catch(j)
{
b.showError("\u6DFB\u52A0\u90AE\u7BB1\u5E10\u53F7\u5931\u8D25...");
}
}
}});
}
...
b.loadCssFile("$css_path$login/bindAccount21b582.css",true,b.document);
...
if(!this._msVer)
{
var a=getTop().getRes("$css_path$qui_dialogPtlogin2275b0.css"),b=(/qui_dialogPtlogin(.*)\.css/.test(a)&&RegExp.$1)||"";
this._msVer=b;
}
return this._msVer;
},getDefaultCfg:function(){
...
return function(f,e){
var c=a.S("importFrame"),d=a.S("dialogPtloginTitle"),b=a.S("geCancelBtn");
if(c)
{
c.style.height=e+'px';
if(e>=199&&e<=298)
