...
//亚马逊men hover延迟插件
//jiel@jumei.com
//search "jQuery-menu-aim" in google
(function($) {
...
(function($) {
    ...
    /*懒加载textarea  注意 css属性必须设置visibility:hidden 如果设置display:none会获取不到textarea的位置*/
    $.fn.lazyload_textarea = function(options) {
        var settings = {
            threshold    : 0,
            failurelimit : 0,
            event        : "scroll",
        ...
        /* Fire one scroll event per scroll. Not one scroll event per textarea. */
        var elements = this;
        ...
        if ("scroll" == settings.event) {
            ...
            var lazyload_on_scroll = function() {
                ...
                /*Remove textarea from array so it is not looped next time.*/
                var temp = $.grep(elements, function(element) {
                    return $(element).hasClass("lazyload");
                });
                elements = $(temp);
        ...
        this.each(function() {
            var self = this;
            $(self).one("appear",function(){
                 $(self).parent().html($(this).val());
                 $(self).removeClass("lazyload");
            })

        });

        /* Force initial check if textarea should appear. */
        $(settings.container).trigger(settings.event);
...
(function($){
    imageSlidePlayer = function(object,config){
        ...
        var _this = this;
        var t;
        var defaults = {fontsize:12,left:10,bottom:15,time:5000,autosize:0};
        ...
        this.factory = function(){
            ...
            $(this.obj).prepend("<div class='img_slider_trigger' style='position:absolute;z-index:20;left:"+this.config.left+"px;bottom:"+this.config.bottom+"px'></div>");
