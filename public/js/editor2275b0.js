...
b.prototype.insertImage=function(f,e){
var g=this;
this.execCmd("InsertImage",f,function(){
var j=g._moEditDoc.selection,h;
if(j&&j.type=="Control")
{
h=g.getSelectionElement();
var i=g._moEditDoc.body.createTextRange();
...
var i=g._moEditWin.getSelection().getRangeAt(0);
h=i.startContainer.childNodes[i.startOffset-1];
}
g.autoScaleImg(h,f);
typeof e=="function"&&e.call(g,f,h);
typeof g.onafterinsertimage=="function"&&g.onafterinsertimage(_asPicUrl);
});
};
...
if(b.FUNCLIB[k])
{
j.funcObj=new b.FUNCLIB[k]({oParamSet:f._moParamSet,editor:f,uiType:i});
j.funcObj._setup({container:j.funcArea,uiType:i});
}
else if(f._moInfile[k])
{
e[f._moInfile[k]]=true;
}
else{
a.debug('editor not defined '+k);
}
}
});
}
...
var s=this,q=s._moEditBody,t=s._moTextBody,j=s._mnAutoRisizeMinHeight,l=s._mDefHeight||j||0;
if(j&&(f||!s._mbNoEditScroll||(q.scrollHeight<=l&&t.style.display=="none")||(t.scrollHeight<=l&&t.style.display=="block")))
{
var r=s.getEditorArea(),p=s._moEditorAreaWin.document,h=a.bodyScroll(p,'clientHeight'),n=a.bodyScroll(p,'scrollHeight')+(a.gbIsIE&&a.gnIEVer===6?10:0),k=r.clientHeight,g=h-n+k;
g=g<j?j:g;
g!=k&&(r.style.height=g+"px");
s._mDefHeight=s._moEditBody.clientHeight;
if(a.gnIEVer===6)
{
a.GelTags("td",s._moEditorAreaObj)[1].style.height=(g+"px");
}
if(s._moResizeEditorHeightList.length>0)
{
var m,o=s._moResizeEditorHeightList;
try{
m=a.calcPos(r,'json').top;
}
catch(u)
{
return;
...
b.CONST.FILES={"$js_path$/com/kits/qmeditor/base/js/editor_toolbar208055.js":"Separate Bold Italic Underline FontName FontName FontSize ForeColor BackColor AlignMode Serial Indent Quot CreateLink SourceEdit Preview SpellCheck UploadEml","$js_path$/com/kits/qmeditor/base/js/editor_toolbar_plus1eeb06.js":"ClipImg Photo ScreenSnap Mo Music"};
b.CONST.FUNCLIST={};
...
m._moMenuCfg=d;
m._moMenu=new a.QMMenu({oEmbedWin:this._moBindEditor.getEditorAreaWin(),nWidth:i||"auto",sWidthDetect:"float",nItemHeight:(d?d.nItemHeight:this._moUiConfig.itemHeight)||b,nArrowPos:g?g<10?10:g:0,nX:j+(g?(m._msUiType=="text"?25:8):0),nY:l[2]+(g?-4:5),oItems:k,bAutoClose:d.bAutoClose||k.length>1,bProxyScroll:d.bAutoClose||k.length>1,onload:function(){
m._mnShowCount++;
m.initMenu(this,d);
},onshow:function(){
m.doMenuShow(this,d);
...
var h=g._moBindEditor.getSelectionElement(),i=g._moEditMenuBar;
if(a.isShow(a.S("QMEditMenuModify",f)))
{
return;
}
...
var j=g._checkEditMenuType(h);
if(j&&j.sHtml!="")
{
if(g._moCurrentEditObj==j.oEditObj)
{
return;
}
i.innerHTML=j.sHtml;
g._moCurrentEditObj=j.oEditObj;
if(!a.isShow(i))
{
g._adjustEditMenuPos(j.oEditObj);
}
else{
a.show(i,true);
}
...
var t=a.S("QMEditMenuInput",e),y=a.getEventTarget(j);
if(y)
{
var A=a.attr(y,"func");
switch(A)
...
if(!a.gbIsIE)
{
var x=f._moEditWin.getSelection(),w=f._moEditDoc.createRange();
if(a.gbIsOpera&&h._moCurrentEditObj.nodeName=="IMG")
{
w.selectNode(h._moCurrentEditObj);
x.removeAllRanges();
x.addRange(w);
}
else if(h._moCurrentEditObj.nodeName=="A")
{
w.selectNode(f.moveToAncestorNode("A"));
x.removeAllRanges();
x.addRange(w);
}
}
...
case "modify":
{
var u=a.S("QMEditMenuLink",e),p=a.S("QMEditMenuCommon",e),v=a.S("QMEditMenuModify",e);
a.show(p,false);
a.show(v,true);
f.saveRange();
t.value=u.href;
...
case "save":
{
var u=a.S("QMEditMenuLink",e),p=a.S("QMEditMenuCommon",e),v=a.S("QMEditMenuModify",e),z=t.value,r=h._moCurrentEditObj,o=r.nodeName=="A"?r:r.parentNode;
if(/[a-zA-Z_0-9.-]+@[a-zA-Z_0-9.-]+\.\w+/.test(z))
{
z=a.trim(z).toLowerCase().indexOf("mailto")==0?z:"mailto:"+z;
}
else{
z=a.trim(z).indexOf("://")==-1?"http://"+z:z;
}
o.href=u.href=z;
u.innerHTML=getTop().htmlEncode(z);
f.loadRange();
a.show(p,true);
a.show(v,false);
break;
...
case "drag":
{
var s=y,n=j.clientX,q=e.document;
var l=function(){
removeEvent(q,"mousemove",k);
removeEvent(q,"mouseup",l);
a.callBack.call(h,h._fView,[y,j,A,parseInt(y.style.left)]);
};
var k=function(B){
var G=parseInt(y.style.left),D=G+(B.clientX-n),C=false;
if(D<-5||D>65)
{
C=true;
}
...
default:
{
a.callBack.call(h,h._fView,[y,j,A]);
}
}
}
if(!a.isShow(t))
...
var m=this,k=m._moBindEditor,i=null,j=null,g=true,h=0,p="",q="",r="",n=QMEditor.EditMenu._TEMPLATE,o={"1":n._IMG_MENU_BAR_LINK,"2":n._IMG_MENU_BAR_IMG,"3":n._IMG_MENU_BAR};
if(!f)
{
...
else if(f.parentNode&&f.parentNode.nodeName=="A")
{
i=f.parentNode;
}
...
(h>1)&&(q=m._getImgMenuContent(j,n));
p=o[h]?o[h].replace({"link":((i&&i.href)||""),"images_path":k._msResPath,"imgcontent":q}):"";
...
if(g&&(a.attr(g,"notForEdit")||a.attr(g.parentNode,"notForEdit")))
{
return;
}
