...
_goStatic._TEMPLATE._MSG=T(['<div style="text-align:center;">','<img src="$images_path$ico_loading1104474.gif" style="display:$dispload$;"/>','$msg$','</div>']);
...
setClass(c,d._moConfig.sClassName+' '+(b?"qm_dialog_flash":""));
},S:function(b){
...
this.configHelp_(b,{bAutoClose:true,nZIndex:1121,nWidth:"auto",nMinWidth:100,nMaxWidth:9999,bAnimation:true,nMaxItemView:1000000,sClassName:b.sClassName?b.sClassName:"qmpanel_shadow rounded5",nArrowDirection:"Up"});
...
b.onclick=function(){
if(b.className.indexOf("btn_disabled")!=-1)
{
return false;
}
...
var D=a.nCurFolderId;
var F=a.sCurSaveFrom;
var f=a.bShowNode;
var H=typeof (a.sTitle)=="undefined"?"\u53EF\u9009\u62E9\u90AE\u7BB1\u522B\u540D\u6216POP\u6587\u4EF6\u5939\u7684\u90AE\u4EF6\u5730\u5740&#10;\u4F5C\u4E3A\u53D1\u4FE1\u5E10\u53F7\u3002":a.sTitle;
var G=typeof (a.sDesContent)=="undefined"?"\u53D1\u4EF6\u4EBA\uFF1A":a.sDesContent;
...
var O=S("sendmailname_val",this._moWin);
if(!O)
{
return;
}
var R="",M=GelTags("span",S("sendmailname_val",this._moWin))[0],N=GelTags("b",S("sendmailname_val",this._moWin))[0],Q=M.getAttribute("nickname"),L=Q?Q.split("|"):[];
if(L[0]==e.email)
{
R=L[1];
}
M.innerHTML=this._TEMPLATE._EMAIL_DISP.replace(e);
N.innerHTML=htmlEncode(R)||e.nick.replace(/(^\"|\"$)/g,"");
S("sendname",this._moWin).value=htmlDecode(N.innerHTML);
this._setSender(e);
if(e.sms)
{
loadJsFileToTop(["$js_path$qmtip207d36.js"]);
var P=this._moWin;
waitFor(function(){
return QMTip;
},function(U){
if(U&&S("sendmailname",P).value==e.email)
{
QMTip.show({tipid:10001,domid:'sendmailname_val',win:P,msg:['<span class="black">\u5C06\u4F7F\u7528\u624B\u673A\u53F7\u90AE\u7BB1\u53D1\u4FE1\uFF0C\u8FD9\u6837\u5BF9\u65B9\u56DE<br/>\u4FE1\u60A8\u5C31\u4F1A\u83B7\u5F97\u77ED\u4FE1\u63D0\u9192\u3002<a onclick="window.open(\'http://service.mail.qq.com/cgi-bin/help?subtype=1&&id=8&&no=1000605\')">\u8BE6\u60C5</a></span>'].join(''),arrow_direction:'down',arrow_offset:25,height_offset:4,tip_offset:-110,width:305,auto_hide:1,notlog:true,bForceShow:true});
}
});
}
callBack.call(this,a.onclickItemCallBack,[e]);
};
var C=typeof (a.sAlignType)=="undefined"?"left":C,I=a.sVerAlign||"bottom";
var h=300;
...
j=l-25;
var B=Math.floor(m*23/h);
var A=Math.floor(m*20/h);
var o=[];
var v=[];
var w=null;
var r=this._CONST;
...
var O=L.email;
var N=O.split("@").pop();
var e=r.hasOwnProperty(N);
var M={nick:L.nickname&&"\""+L.nickname+"\"",email:O,phone:L.phone,emaildisp:this._mbIsAutoWidth?O:subAsiiStr(O,e?A:B,"..."),signid:L.signid,domain:s.replace({images_path:getPath("image"),margin_top:(L.phone==1?416:(e?r[N]:321))}),sms:L.phone==1&&L.smsleft>0};
y.push(M);
o.push(extend({smtp:L.smtp==2?'':''},M));
v.push(function(P){
z._mfOnclickItemCallBack(M);
});
if(!D&&!F&&getTop().goUserInfo.get('RealDefaulSender')==O)
{
w=M;
}
else if((F&&F.toLowerCase()==L.email)||(!F&&D&&D==L.folderid)||w==null)
{
w=M;
}
})();
}
p.innerHTML=n.replace({title:H,desContent:G,email_width:j,sel_width:l,width:m,images_path:getPath("image"),nick:w.nick,email:t.replace(w)});
var k=0;
...
{
var L=getStrDispLen(q[Y].email+(q[Y].smtp==2?'':''));
if(k<L)
{
k=L;
}
}
k=Math.max(this.clientWidth,k+42);
}
var U=calcPos(this),Q=[],O=GelTags("span",this)[0],M=0;
for(var Y=0;Y<o.length;Y++)
{
O.innerHTML==o[Y].email&&(M=Y);
Q.push({sId:Y,sItemValue:u.replace(extend({selected:O.innerHTML==o[Y].email},o[Y]))});
}
var P=this.getElementsByTagName("span")[0],X="",V=P&&P.innerHTML||"";
for(var Y=0;Y<q.length;Y++)
{
if(q[Y].email==V)
{
X=q[Y].nickname;
var W=P.getAttribute("nickname"),N=W?W.split("|"):[];
if(N[0]==V)
{
X=N[1];
}
...
Q.push({sId:"border",nHeight:5,sItemValue:['<div style="border-top:1px solid;margin-top:2px;padding-top:2px;font-size:0;line-height:0;height:0;"></div>'].join(""),bDisSelect:true});
Q.push({sId:"send__nickname",nHeight:25,sItemValue:['<div style="padding-left:15px;overflow:hidden">','<span id="snn_getAlias" style="line-height:25px;">','<font style="color:#000">\u53D1\u4FE1\u6635\u79F0\uFF1A</font>','<span title="',X,'">',subAsiiStr(X,16,"...",true),'</span>','&nbsp;<a href="javascript:;" onclick="getTop().QMSender.initAlias(\'',X,'\');return false">\u4FEE\u6539</a>&nbsp;&nbsp;&nbsp;&nbsp;','</span>','<span id="snn_setAlias" style="display:none;line-height:25px;">','<input onkeyup="if(event.keyCode==13){getTop().QMSender.setAlias()};" class="text" onchange="" style="width:120px" type="text" value=""/>','&nbsp;<input type="button" value="\u786E\u8BA4" onclick="getTop().QMSender.setAlias();return false" />','</span>','</div>'].join(""),bDisSelect:true});
var e=Q.length>17?17:Q.length,R=new (getTop().QMMenu)({oEmbedWin:z._moWin,sId:"sendermenu",nMaxItemView:17,nX:C=="left"?U[3]:U[3]-(k-this.clientWidth),nY:I=="bottom"?U[2]:(U[2]-21*e-21),nMinWidth:230,bAnimation:false,nItemHeight:21,oItems:Q,onitemclick:function(Z){
v[Z]();
},onshow:function(){
R.selectItem(M);
}});
...
addEvents(d,c);
e._moMenu=new QMAutoComplete._QMAutoCompleteMenu({sId:unikey(),oItems:[],supportKey:e._mbsupportKey,oEmbedWin:e._moWin,nWidth:b.nWidth||"auto",nMinWidth:b.nMinWidth||100,nItemHeight:b.nItemHeight||21,nMaxItemView:b.nMaxItemView||0,type:b.type,sClassName:b.sClassName,oClass:b.oClass,bDisplay:false,onselect:function(f){
...
QMAjax.send(a.sUrl||'/cgi-bin/mail_mgr',{content:a.oQuery.join(""),onload:function(c,e,d){
if(c)
{
var f=evalValue(e);
if(e.indexOf("cgi exception")==-1)
{
a.onload(f,e,d);
return;
}
else if(f&&f.errcode=="-2")
{
...
var f=a.bML,m=a.oMail,j=m.length,r=unikey('mv'),p=[a.bML?'&location=mail_list':''],g=b=="new",e=false,q;
...
var t=g?"\u5DF2\u5C06\u90AE\u4EF6\u6210\u529F\u79FB\u52A8":T("\u5DF2\u5C06\u90AE\u4EF6\u6210\u529F\u79FB\u52A8 <a href='/cgi-bin/mail_list?sid=$sid$&folderid=$folderid$&page=0' style='color:white' onclick='getTop().gotoCheckMovedEmail();' target='mainFrame' >[\u67E5\u770B]</a>").replace({sid:getSid(),folderid:b});
AjaxSendMailMgr({oQuery:p,sSuccessMask:'mail_move successful',sDefErrMsg:'\u79FB\u52A8\u90AE\u4EF6\u5931\u8D25\uFF0C\u8BF7\u91CD\u8BD5',onload:function(u,w,v){
var x,z;
if(a.oWin[r])
{
x=callBack(a.onbeforesend,[{sucMsg:t+getRollbackText(u.rbkey)}]);
}
if(g)
{
var A=evalValue(w);
z={sDestName:q,sDestId:(A&&A.sDesId)||""};
}
var y=callBack(a.oncomplete,[a,u,z]);
!(x&&y)&&u.msg&&showInfo(u.msg+getRollbackText(u.rbkey));
...
if(g)
{
promptFolder({type:'folder',onreturn:function(t){
p.push('&foldername=',t);
q=t;
h();
}});
...
function doRmMail(b,c,a)
{
var f=b.sDelRetMsg||"\u5220\u9664\u6210\u529F",e=b.sDelIngMsg||"\u90AE\u4EF6\u5220\u9664\u4E2D...",d=b.sFid,g=unikey('rm');
if(b.bPop&&getTop().goUserInfo.get("POP_PROPOSE"))
{
confirmBox({title:"\u90AE\u7BB1\u529F\u80FD\u63A8\u8350",mode:"prompt",msg:T(['<div class="dialog_f_t">\u5728$dn$\u90AE\u7BB1\u4E2D\u5220\u9664\u90AE\u4EF6\uFF0C\u540C\u65F6\u4E5F\u5220\u9664\u539F\u90AE\u7BB1\u4E2D\u7684\u5BF9\u5E94\u90AE\u4EF6?</div>','<div class="dialog_f_d">','\u60A8\u4E5F\u53EF\u4EE5\u8FDB\u5165\u201C\u4FEE\u6539\u8BBE\u7F6E\u201D\u4E2D\u8BBE\u7F6E\u3002','<a href="http://service.mail.qq.com/cgi-bin/help?subtype=1&&id=26&&no=326" target="_blank" >\u4E86\u89E3\u8BE6\u8BF7</a>','</div>']).replace({dn:getDomain(true)}),onreturn:function(h){
if(h)
{
runUrlWithSid(T("/cgi-bin/foldermgr?fun=updpop&updflag=22&folderid=$folderid$").replace({folderid:d}));
...
showInfo(e,-1);
b.oWin[g]=1;
AjaxSendMailMgr({oQuery:c,sSuccessMask:'mail_del successful',sDefErrMsg:'\u5220\u9664\u90AE\u4EF6\u5931\u8D25\uFF0C\u8BF7\u91CD\u8BD5',onload:function(h,k,j){
f=f+getRollbackText(h.rbkey);
var l;
if(b.oWin[g])
{
l=callBack(b.onbeforesend,[{sucMsg:f}]);
}
var n=h.url,m=callBack(b.oncomplete,[b,h]);
!(l&&m)&&showInfo(f);
if(!m&&b.oWin[g])
{
dealRmMailJump(b,h);
...
var n=c.join("");
if(a||isSelectAllFld(b.oWin)||b.sFid==3||b.sFid==4||b.sFid==1||n.indexOf("mail_grpsearchall")>-1||n.indexOf("mail_grpoprall")>-1)
{
reloadLeftWin();
}
if(b.oMail.length>4)
{
setMailFilter({sFilterType:"delete"},b);
}
setRollBack(h.rbkey,'\u5220\u9664\u90AE\u4EF6');
}});
}
...
},apply:function(_asItemId,_asItemValue){
...
if(!m.get("nSize")||!m.get("nUploadPercent"))
{
return;
}
if(!m._nLastProcssTime)
{
m._nLastProcessPercent=m.get("nUploadPercent");
m._nLastProcssTime=n;
return;
}
if(n-m._nLastProcssTime>(l||1000))
{
...
var m=this;
if(l.get("nUploadPercent")>=100&&l.get("nPercent")>=80)
{
return;
}
m.calcSpeed(l);
l.set({sStatus:"uploading",sUploadStep:"posting",nPercent:l.get("nUploadPercent")*0.8});
...
if(l.get("nPercent")>=99)
{
clearInterval(l._nWaitProcess);
return;
}
m.callBack("onprocess",[l.set({"nPercent":l.get("nPercent")+1,"sUploadStep":"waiting"})]);
},100);
}
...
var b=a.T(['cursor: pointer;','font-family: Times;','position: absolute;','cursor: pointer;','width:2000px;','height:600px;',a.isBrowser("ie")?'font-size:500px;':'',a.isBrowser("ff")&&parseInt(getTop().gsFFVer)<22?'right:426px;':'right:0px;']);
var d=function(j){
...
var l=g.sFlashUrl||a.getRes("$swf_path$uploader1d35b7.swf");
...
j.set({nUploadedSize:Math.floor(j.get('nSize')*h/100),nUploadPercent:h});
k.onprocess(j);
...
var r=this,p=r.getFile(r.getFlashFileId(m)),q=r.getFlash();
if(p)
{
p.set({nUpType:q.getCurUptype(),nPostMode:n,nRetryTimes:q.getRetryTimes()});
}
k.onFlashComplete_.apply(r,arguments);
},onFlashError_:function(m,p,o,n){
var s=this,q=s.getFile(s.getFlashFileId(m)),r=s.getFlash();
if(q)
{
q.set({nUpType:r.getCurUptype(),nPostMode:r.getCompleteMode(),nRetryTimes:r.getRetryTimes()});
}
k.onFlashError_.apply(s,arguments);
},defaultQueryData:function(){
var m=this.oCfg;
...
var c=function(k){
return {uploadInChip:function(l){
var m=this;
if(!m.prepareUpload_(l))
{
return false;
}
m.callBack("onprocess",[l.set({nTry:l.get("nTry")+1,nUpTime:new Date().valueOf(),bChipFile:true,sStatus:"uploading",sUploadStep:"signing",nSignPercent:0})]);
m.doChipFileSign_(l);
return true;
},onChipFileForNormalAttachSignEnd_:function(l,m){
var n=this;
m.sUploadStep='creating';
n.callBack("onprocess",[l.set(m)]);
a.QMAjax.send(['/cgi-bin/uploadunite?func=CreateFile&&inputf=json&outputf=json&&sid=',a.getSid()].join(""),{method:'post',headers:{"If-Modified-Since":"0","Cache-Control":"no-cache, max-age=0"},content:a.T('{"path":"$path$","appid":"$appid$","size":$size$,"md5":"$md5$","sha":"$sha$","sha3":"$sha3$"}').replace({appid:l.get("nAppId"),path:encodeURIComponent(l.get("sName")),size:l.get("nSize"),md5:l.get("sMD5"),sha:l.get("sSHA"),sha3:l.get("sSHA3")}),onload:function(o,q,p){
var s=a.evalValue(q||"{}");
if(o&&s.result.errCode=="0")
{
l.set(s.data);
n.onChipUpload_(l);
}
else{
var v=l.get("sId"),u=["SPUploader",v].join(""),t=getTop(),r=t.getMainWin().QMAttach;
t.show(r.S(u),false);
r.retryUpload(v);
}
}});
},doChipFileSign_:g,onChipUpload_:function(l){
var n=this,m=new Date().valueOf();
l.set({sStatus:"uploading",sUploadStep:'posting',bChipFile:true,nStartTime:m,nRealPostTime:0,nRealPostSize:0,nUpTime:m});
n.doChipUpload_(l);
n.onprocess(l);
},doChipUpload_:g,onChipUploadEnd_:function(l){
var m=this;
m.onprocess(l.set("nUploadPercent",100));
m.fCancel=null;
},onChipFileSignEnd_:function(l,m){
var n=this;
if(l.get("bIsUptoFtnForNormal"))
{
n.onChipFileForNormalAttachSignEnd_.apply(n,arguments);
}
else{
n.onChipFileSignEndFtn_.apply(n,arguments);
}
},onChipFileSignEndFtn_:function(l,m){
var n=this;
m.sUploadStep='creating';
n.callBack("onprocess",[l.set(m)]);
a.QMAjax.send(a.T("/cgi-bin/ftnCreatefile?uin=$uin$&ef=js&resp_charset=UTF8&s=ftnCreate&sid=$sid$&dirid=$dirid$&path=$path$&size=$size$&md5=$md5$&sha=$sha$&sha3=$sha3$&appid=$appid$&loc=$loc$").replace({sid:a.getSid(),loc:["ftnCreatefile","ftnCreatefile","ftnCreate",(l.get("sFrom")||"")+l.get("nAppId")].join(","),path:encodeURIComponent(l.get("sLocalPath")||l.get('sName')),dirid:l.get("sPathId"),size:l.get("nSize"),md5:l.get("sMD5"),sha:l.get("sSHA"),sha3:l.get("sSHA3"),appid:l.get("nAppId")}),{method:"GET",headers:{"If-Modified-Since":"0","Cache-Control":"no-cache, max-age=0"},onload:function(o,q,p){
var r=a.evalValue(q||"{}");
if(o&&r.errcode=="0")
{
l.set(r.data);
...
n.onerror(l.set({nPercent:0,sError:(r&&r.errcode)?n.err("cgi",r.errcode,r.appname):n.err("http",q=='abort'?-1:p.status)}));
}
}});
},onChipFileSigning_:function(m,l){
var n=this;
m.set("nSignPercent",Math.max(Math.min(100,Math.floor(l)),0));
n.callBack("onprocess",[m]);
},chipUpdateFileSize_:function(m,l){
var o=this,n=new Date().valueOf();
if(!m._nUpFileSizeTime)
{
m._nUpFileSizeTime=n;
}
else if(n-m._nUpFileSizeTime>=(l||5000))
{
a.QMAjax.send(a.T("/cgi-bin/ftnUpFileSize?sid=$sid$&filekey=$fileid$&uploadsize=$uploadsize$&ef=js").replace({sid:a.getSid(),fileid:m.get("sFileId"),uploadsize:m.get("nUploadedSize")}),{method:"GET",headers:{"If-Modified-Since":"0","Cache-Control":"no-cache, max-age=0"},onload:function(p,r,q){
if(r.indexOf("<!--cgi exception-->")!=-1)
{
var u=a.evalValue(r||"{}");
var t=o.getFile();
for(var v in t)
...
o.onerror(s.set("sError",["cgi",u.errcode,"ftnUpFileSize"].join(",")));
}
}
}
}});
...
var m=this;
if(l.get('bChipFile')&&l.get("nUploadedSize")&&!l.get("bIsUptoFtnForNormal"))
{
m.chipUpdateFileSize_(l);
}
k.onprocess.call(m,l);
}};
};
var e=function(k){
var l=h.FlashPopup.prototype,m=h.FlashPopupMail.prototype;
return a.extend(c(k),{init_:function(n){
var o=this;
k.init_.call(o,n);
},doChipFileSign_:function(n){
var p=this,o=p.getFlash();
n.set({nSignChunkSum:Math.ceil(n.get('nSize')/p.nLoadFileBlockSize),nCurrentSignChunk:0});
n.fCancel=function(){
o.cancelFTNUpload();
n.bCancelH5Sign_=true;
};
o.ftnUploadInit();
...
o.nLoadFileBlockSize=2*1024*1024;
n.ftnUploadInit();
o.nFlashFileIndex=0;
...
},onFlashError_:m.onFlashError_,onFlashLoadH5FileData_:function(n,o){
var p=this;
...
q=o.get('nCurrentSignChunk')*r.nLoadFileBlockSize;
p=Math.min(q+r.nLoadFileBlockSize,o.get('nSize'));
r.getH5FileReader_(o,q,p,r._onLoadSignFileData);
}
else{
if(o.bCancelH5Upload_)
{
return;
}
q=parseInt(n,10);
o.set({nUploadPercent:q/o.get('nSize')*100,nUploadedSize:q});
...
var u=t.target.error;
s.onerror(p.set({nUpType:s.getFlash().getCurUptype(),sError:s.err("html5fd",u.code,u.name)}));
};
r.readAsDataURL(s.getFileSliceFunc_().call(p.oH5File,o,n));
return r;
},cancel:m.cancel});
};
j.getChipLib=c;
...
{
n.uploadToFtn=m;
if(getTop().goExpers.uploadunite===true&&getTop().getMainWin().getPageId&&getTop().getMainWin().getPageId()==="compose")
{
n.upload=function(){
...
var n=this,m=n.getFlash(),o='http://'+l.get('sIP')+':'+l.get('nPort');
