...
var QMIme={_JS:"//ime.qq.com/fcgi-bin/getjs ",_CONF:{"ime-cfg":"lt=102"},_detect:function(a){
...
(b?showInfo:showError)([b?"\u4E91\u8F93\u5165\u6CD5\u6210\u529F\u542F\u52A8":"\u4E91\u8F93\u5165\u6CD5\u542F\u52A8\u5931\u8D25 - <a href='http://service.mail.qq.com/cgi-bin/help?subtype=1&&id=10000&&no=1000561' target='_blank'>\u5E2E\u52A9</a>"].join(""));
}));
},toggle:function(){
...
{
var m=d.ownerDocument,p=m.defaultView||m.parentWindow,b=e.clientX+bodyScroll(p,'scrollLeft'),c=e.clientY+bodyScroll(p,'scrollTop'),a=BaseMailOper,j=a.getInstance(p),k=GelTags('input',d)[0],h=k.checked==true;
if(!j)
{
return;
}
...
function i(r)
{
var A=[],C=[],D=[],y=0,B=r.oMail,u=r.sFid=="11"||/^(LP|ZP)/.exec(B[0].sMid),x=B.length,s=false,z={nHeight:10,sItemValue:'<div style="background:#CCC; height:1px; margin-top:7px; overflow:hidden;"></div>',bDisSelect:true},H=T(['<div style="white-space:nowrap;zoom:1;">','<input type="button" class="item_square flagbg$flagbg$"/>','<span class="item_square_txt">$name$ &nbsp</span>','</div>']);
getTop().E(B,function(J){
if(J.sMid=='AC0000-00000000000000000000000')
{
s=true;
}
});
...
if(!s)
{
A.push({sId:"preview",sItemValue:'<div class="right"><img src="/zh_CN/htmledition/images/spacer.gif" class="arrow_meunico" /></div>\u9884\u89C8',oSubMenu:{sClassName:'rightpre qmpanel_shadow',nWidth:460,oItems:[{nHeight:350,sItemValue:T('<div scroll="true" id="pre" style="height:350px;width:438px;overflow-x:hidden;overflow-y:auto;"><div style="*margin:100px 0 0 0;height:64px;text-align:center;width:100%;"><center><img src="$images$ico_loading2.gif"/></center></div></div>').replace({images:getPath("image")})}],onload:function(){
var J=this,K=r.oMail[0].sMid;
QMAjax.send(T('/cgi-bin/readmail?sid=$sid$&folderid=$f$&t=quickreadmail&mailid=$m$&mode=preview&maxage=3600&ver=$v$').replace({v:rdVer(K,0),sid:getSid(),f:r.sFid,m:K}),{method:'GET',onload:function(L,N,M){
if(L)
{
...
O.innerHTML=filteScript(N).replace(/<style ?.*>[\s\S]*?<\/style>/ig,"");
swapLink(O,"preview",O.ownerDocument);
}
}
}});
}}});
}
A.push({sId:"opennew",sItemValue:'\u65B0\u7A97\u53E3\u6253\u5F00'});
A.push(z);
}
var v=false;
for(var I=0;I<x;I++)
{
y+=B[I].bUnr?1:0;
B[I]&&B[I].bSys==1&&(v=true);
}
if(r.sFid!=5&&r.sFid!=6)
{
A.push({sId:"delmail",sItemValue:'\u5220\u9664'});
}
else{
A.push({sId:"predelmail",sItemValue:'\u5F7B\u5E95\u5220\u9664'});
}
if(!s)
{
if(!v&&l.bSpam&&r.sFid!=6&&!u)
{
A.push({sId:"spammail",sItemValue:"\u8FD9\u662F\u5783\u573E\u90AE\u4EF6"});
}
}
A.push(z);
if(l.bReadMode)
...
if(y)
{
A.push({sId:"read",sItemValue:"\u6807\u8BB0\u4E3A\u5DF2\u8BFB"});
}
if(y!=x)
{
A.push({sId:"unread",sItemValue:"\u6807\u8BB0\u4E3A\u672A\u8BFB"});
}
}
if(x==1&&r.sFid!=14&&!s)
{
A.push({sId:"createreceiverule",sItemValue:"\u521B\u5EFA\u6536\u4FE1\u89C4\u5219"});
}
if(l.bTagMode&&!u)
{
var t=0;
...
if(t)
{
C.push({sId:'rmalltag',sItemValue:'\u53D6\u6D88\u6807\u7B7E'},extend({sId:'deltaghr'},z));
}
for(var G=QMTag.get(),I=0,w=G.length;I<w;I++)
{
var F=G[I];
C.push({sId:'tid_'+F.id,sItemValue:H.replace(F)});
}
if(w)
{
C.push(z);
}
C.push({sId:'newtag',sItemValue:'\u65B0\u5EFA\u6807\u7B7E'});
A.push({sId:"mark",sItemValue:'<div class="right"><img src="/zh_CN/htmledition/images/spacer.gif" class="arrow_meunico" /></div>\u6807\u7B7E',oSubMenu:{oItems:C}});
}
if(!u)
{
A.push({sId:"move",sItemValue:'<div class="right"><img src="/zh_CN/htmledition/images/spacer.gif" class="arrow_meunico" /></div>\u79FB\u52A8\u5230',oSubMenu:{oItems:l.moMoveItem}});
}
return A;
}
var n;
if(l.mnFolderType==0||l.mnFolderType==4)
{
n=QMMailList.getCBInfo(l.oWin);
QMMailList.selectedUI(n);
if(l.mnFolderType==4)
{
return;
}
...
n=l.oWin.QMReadMail.getTag();
}
ossLog("delay","all","stat=right");
j.setMailInfo(n);
var f=null,q=unikey(),o=new QMMenu({sId:q,oEmbedWin:p,nWidth:"auto",nMaxWidth:180,nMaxItemView:14,sClassName:"qmpanel_shadow",bAnimation:false,bAutoClose:true,oItems:i(n),onitemclick:function(s,r){
ossLog("delay","all","stat=right&opr="+s);
j.apply(f=s,r.sItemValue);
},onbeforeclose:function(){
if(this.option("sId")==q)
{
if(!h&&(f==null||'|spammail|new|newtag|autotag|'.indexOf(f)<0))
{
try{
k.checked=false;
var r=QMMailList.getCBInfo(p);
QMMailList.selectedUI(r);
}
catch(s)
{
}
...
if(t.option("sId")==q)
{
var r=parseInt(t.option("nHeight")),s=parseInt(t.option("nWidth")),u=calcAdjPos([c,b,c,b],s,r,p,1);
t.option("nX",Scale.fixOffsetLeft(u[3])).option("nY",Scale.fixOffsetTop(u[0]));
}
}});
if(l.mnFolderType==0)
{
var g=n.oMail.length;
o.itemOption("rmalltag","bDisplay",g);
o.itemOption("deltaghr","bDisplay",g);
}
}
(function(a){
...
;(function(){
if(window.gWebpSupport===undefined||window.gWebpSupport===3)
{
return;
}
