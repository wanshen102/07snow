...
var _oNetDisk={},_oDiskStat={nNum:0,nMax:3,oBound:[]},_oMenuData={"weiyun":{sAlias:"weiyun",sLabel:"\u5FAE\u4E91",sUserid:"",limit:1024*1024*1024,over:"\u6587\u4EF6\u5927\u5C0F\u4E0D\u8D85\u8FC71G"},"dropbox":{sAlias:"dropbox",sLabel:"Dropbox",sUserid:""}},_oTop=getTop(),_oProgressFunc={},_TIMER_MAX=30,_TIMER_INTERVAL=6*1000,_nTimerId=0,_nTimerCnt=_TIMER_MAX,_goLocalInfo;
...
var _oDoc=_aoDom.ownerDocument,_sMenuId="netdisk_droplist",_oPos=_oTop.calcPos(_aoDom,"json"),_oNetDisk=this.netDiskMgr,_oNetDiskInfo=_oNetDisk.getInfo(),_oMainFramePos=_oTop.calcPos(_oTop.S("mainFrame"),"json"),_oMenuItems=_oTmpl["bindDisk"].replace({sid:_oTop.getSid(),from:"compose",info:[_oNetDisk.getInfo("weiyun")&&_oTop.extend({images_path:_oTop.getPath("image")},_oNetDisk.getInfo("weiyun"))||{images_path:_oTop.getPath("image")},_oNetDisk.getInfo("dropbox")&&_oTop.extend({images_path:_oTop.getPath("image")},_oNetDisk.getInfo("dropbox"))||{images_path:_oTop.getPath("image")}]});
...
var _sUin=_oTop.getUin(),_sUrl=_oTop.T("/cgi-bin/loginpage?t=loginpage_weiyun&weiyunuin=$uin$").replace({uin:_sUin}),_oDlg=new _oTop.QMDialog({sId:"netDiskPtlogin",sTitle:"\u6388\u6743\u5FAE\u4E91",nWidth:350,nHeight:321,sUrl:_sUrl,onshow:function(){
_oTop.QMDialog("netDiskListDlg")&&_oTop.QMDialog("netDiskListDlg").hide();
}});
_oTop.showError("\u4E3A\u4FDD\u969C\u60A8\u7684\u5E10\u53F7\u5B89\u5168\uFF0C\u9700\u8981\u91CD\u65B0\u6388\u6743\u5FAE\u4E91");
_oTop.fAfterLogin=function(){
_afCallback&&_afCallback();
_oDlg.close();
delete _oTop.fAfterLogin;
delete _oTop.fLoginResize;
};
_oTop.fLoginResize=function(_anWidth,_anHeight){
_oDlg.option("nWidth",_anWidth).option("nHeight",_anHeight+40);
};
},"bind":function(_asFrom){
var _oNetDisk=this.netDiskMgr;
new _oTop.QMDialog({sId:"netBindDiskDlg",sTitle:"\u7ED1\u5B9A\u5176\u4ED6\u7F51\u76D8",nWidth:430,nHeight:282,sBodyHtml:_oTmpl["bindDisk"].replace({sid:_oTop.getSid(),from:_asFrom,info:[_oNetDisk.getInfo("weiyun")&&_oTop.extend({images_path:_oTop.getPath("image")},_oNetDisk.getInfo("weiyun"))||{images_path:_oTop.getPath("image")},_oNetDisk.getInfo("dropbox")&&_oTop.extend({images_path:_oTop.getPath("image")},_oNetDisk.getInfo("dropbox"))||{images_path:_oTop.getPath("image")}]})});
...
_aoDom.innerHTML=_oTop.TE(['<span>','$@$if($oList$.length==1)$@$','$oList.0$','$@$else$@$','\u8F6C\u5B58','$@$endif$@$','</span>','$@$if($oList$.length>1)$@$','<span class="bind_down_icon"></span>','$@$endif$@$']).replace({oList:_oItemList});
...
},_fBigAttWeiyun=function(_asFid,_asKey,_asAttachId){
var _sFileName=_asAttachId.split("|").pop();
var _oNewWin=window.open(_oTop.T(["$base_domain$/cgi-bin/readtemplate","?&sid=$sid$&ptype=$type$&t=attachfolder_page_todisk","&fid=$fid$&key=$key$","&filename=$filename$&fileicon=$fileicon$&nocheckframe=true","&action=bigattweiyun"]).replace({base_domain:_oTop.getTopHost(),sid:_oTop.getSid(),type:"weiyun",fid:_asFid,key:_asKey,fileicon:_oNetDisk.dataCenter.getFileIcon(_sFileName.split(".").pop()).replace("/ico_min/","/ico_max/").replace("/ico_mid/","/ico_max/"),filename:encodeURI(_sFileName)}));
},_fCheckFile=function(_aoFileInfo){
var _sName=_aoFileInfo.sFileName.toLowerCase(),_nSize=parseFloat(_aoFileInfo.sSize),_nInvalidFormat={"swf":true,"dat":true,"mov":true,"mp4":true,"3gp":true,"avi":true,"rmvb":true,"wmf":true,"mpg":true,"rm":true,"asf":true,"mpeg":true,"mkv":true,"wmv":true,"flv":true,"f4a":true,"webm":true,"mod":true,"mpe":true,"fla":true,"bov":true,"viv":true,"asx":true,"mts":true,"vob":true,"tp":true,"ts":true,"f4v":true,"dv":true,"m2ts":true};
...
if(_nInvalidFormat[_sName.split(".").pop()])
{
return ["<span class='graytext'>\u8F6C\u5B58\u5230",_oMenuData[i].sLabel,"(\u6682\u4E0D\u5141\u8BB8\u8F6C\u5B58\u89C6\u9891\u6587\u4EF6)</span>"].join("");
}
return "";
};
...
if((_sTmpValue=_fCheckFile({sFileName:_sFileName,sSize:_sSize}))&&i=="weiyun")
{
_bDisabled=true;
}
else{
...
if(_isBigAttach)
{
_fBigAttWeiyun.apply(this,[_sFid,_sKey,_sAttid]);
}
else{
...
_oTop.confirmBox({title:"\u63D0\u793A",msg:_oTop.T("\u5DF2\u6210\u529F\u7ED1\u5B9A\u7F51\u76D8\uFF0C\u70B9\u51FB\u201C\u4E0B\u4E00\u6B65\u201D\u8FDB\u884C\u9644\u4EF6\u8F6C\u5B58\u3002").replace(_aoData.oBound[_asId]),confirmBtnTxt:"\u4E0B\u4E00\u6B65",cancelBtnTxt:"\u53D6\u6D88",width:400,onreturn:function(_aIsOk){
if(_aIsOk)
{
if(_isBigAttach)
{
